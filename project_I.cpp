...
Image::Image(const vector<vector<int>> &r, const vector<vector<int>> &v,
...
Image Image::composanteRouge()const {
  vector<vector<int>> vide(largeur, vector<int> (longueur,0));
...
void menu() {
  ...
  cout << "21. FlouG3 de l'image" << endl;
  cout << "22. FlouG5 de l'image" << endl;
  cout << "23. ContourSobel de l'image" << endl;
...
Image Image::rognerG(int x) const {
  vector<vector<int>> vide1;
  vector<vector<int>> vide2;
  vector<vector<int>> vide3;
  ...
  for (int i = 0; i < largeur; i++) {
    vide1.push_back(vector<int>(longueur-x));
    vide2.push_back(vector<int>(longueur-x));
    vide3.push_back(vector<int>(longueur-x));
  }

  for (int i = 0; i < largeur; i++) {
    for (int j = x; j < longueur; j++) {
      vide1[i][j-x] = rouge[i][j];
      vide2[i][j-x] = vert[i][j];
      vide3[i][j-x] = bleu[i][j];
    }
  }
...
Image Image::rognerD(int x) const {
  vector<vector<int>> vide1;
  vector<vector<int>> vide2;
  vector<vector<int>> vide3;
...
Image Image::rognerH(int x) const {
  vector<vector<int>> vide1;
  vector<vector<int>> vide2;
  vector<vector<int>> vide3;
  ...
  for (int i = x; i < largeur; i++) {
    for (int j = 0; j < longueur; j++) {
      vide1[i-x][j] = rouge[i][j];
      vide2[i-x][j] = vert[i][j];
      vide3[i-x][j] = bleu[i][j];
    }
  }
  return (Image(vide1, vide2, vide3));
}

Image Image::rognerB(int x) const {
  vector<vector<int>> vide1;
  vector<vector<int>> vide2;
  vector<vector<int>> vide3;
...
void modification(int choix,const Image & image, Image& image1){
  ...
  switch (choix) {
    ...
    case BAISSERLUMINOSITE:
        cout << "Baisser la luminosite de l'image de : ";
        cin >> x;
    ...
    case ROGNERG :
        cout << "Combien de colonnes souhaitez vous rogner ?"<< endl;
        cin >> x;
        image1 = image.rognerG(x); break;
    case ROGNERD :
        cout << "Combien de colonnes souhaitez vous rogner ?"<< endl;
        cin >> x;
        image1 = image.rognerD(x); break;
    case ROGNERH :
        cout << "Combien de lignes souhaitez vous rogner ?"<< endl;
        cin >> x;
        image1 = image.rognerH(x); break;
    case ROGNERB :
        cout << "Combien de lignes souhaitez vous rogner ?"<< endl;
        cin >> x;
        image1 = image.rognerB(x); break;
    ...
    case FLOUG3 :{
      ...
      image1 = flouG3.application(image); break;
    }
    case FLOUG5 :{
      ...
      image1 = flouG5.application(image); break;
    }
    case CONTOURSOBEL :{
      ...
      for (int k = -_rayon; k <= _rayon; k++) {
        for (int l = -_rayon; l <= _rayon; l++) {
          if (i + k >= 0 && i + k < largeur && j + l >= 0 && j + l < longueur) {
            vide1[i][j] += image.getRouge()[i + k][j + l] * _action[k + _rayon][l + _rayon];
            vide2[i][j] += image.getVert()[i + k][j + l] * _action[k + _rayon][l + _rayon];
            vide3[i][j] += image.getBleu()[i + k][j + l] * _action[k + _rayon][l + _rayon];
          }
        }
      }
    }
...
Image Image::contourSobel(const vector<vector<float>>& fx,vector<vector<float>>& fy)const{
  ...
  Filtre grX(fx, 1);
  Filtre grY(fy, 1);
  Image Gx = grX.application(*this);
  Image Gy = grY.application(*this);
